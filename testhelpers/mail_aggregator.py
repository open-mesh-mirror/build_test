...
def usage(prog):
    print('%s FILE COMMAND\n' % (prog), file=sys.stderr)
    print('COMMAND:', file=sys.stderr)
    print('\tcreate', file=sys.stderr)
    print('\tadd BRANCH NAME LOGFILE LONGLOGFILE', file=sys.stderr)
    print('\tsend FROM TO TOPIC', file=sys.stderr)
    print('\tadd_buildtests BRANCH VERSION CONFIG', file=sys.stderr)
    sys.exit(1)
...
def create():
    ...
    cur.execute('CREATE UNIQUE INDEX buildtests_unique '
                'ON buildtests(branch, version, config)')
    con.commit()
    cur.close()
...
def add_buildtests():
    ...
    try:
        ...
        con.text_factory = str
        cur = con.cursor()
        cur.execute('INSERT INTO buildtests VALUES (?, ?, ?)',
...
def send():
    ...
    cur.execute('SELECT branch, COUNT(DISTINCT version) FROM buildtests '
                'group by branch')
    versioncount = cur.fetchall()
    cur.execute('SELECT branch, COUNT(DISTINCT config) FROM buildtests '
                'group by branch')
