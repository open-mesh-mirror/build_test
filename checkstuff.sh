...
test_sparse()
{
	...
	(EXTRA_CFLAGS="$extra_flags" "${MAKE}" CHECK="${SPARSE} -Wsparse-all -Wno-declaration-after-statement -Wno-flexible-array-nested -Wno-transparent-union -Wnopointer-arith -Wno-ptr-subtraction-blows -Wno-constexpr-not-const -Wno-flexible-array-array  $extra_flags" $config C=1 KERNELPATH="${LINUX_HEADERS}/${linux_name}" 3>&2 2>&1 1>&3 \
