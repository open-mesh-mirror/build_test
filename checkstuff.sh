...
DEFAULT_LINUX_VERSIONS=$(echo linux-3.{2..19} linux-4.{0..11})
LINUX_VERSIONS=${LINUX_VERSIONS:=${DEFAULT_LINUX_VERSIONS}}
LINUX_DEFAULT_VERSION=${LINUX_DEFAULT_VERSION:="linux-4.11"}
...
check_external()
{
	...
	if [ ! -x "${CPPCHECK}" ]; then
		...
		echo "    git clone -b 1.77 git://github.com/danmar/cppcheck.git cppcheck"
	...
	if [ ! -x "${SMATCH_CGCC}" -o ! -x "${SMATCH}" ]; then
		...
		echo "    git -C smatch reset --hard 203fa17c87a83dd2061f950abb4974d116924bc5"
