...
DEFAULT_LINUX_VERSIONS=$(echo linux-3.{2..19} linux-4.{0..13})
LINUX_VERSIONS=${LINUX_VERSIONS:=${DEFAULT_LINUX_VERSIONS}}
LINUX_DEFAULT_VERSION=${LINUX_DEFAULT_VERSION:="linux-4.13"}
...
check_external()
{
	if [ ! -x "${CGCC}" -o ! -x "${SPARSE}" ]; then
		...
		echo "    git -C sparse reset --hard v0.5.1-28-g23a393b"
		echo "    make -C sparse"
		exit 1
	fi
	...
	if [ ! -x "${SMATCH_CGCC}" -o ! -x "${SMATCH}" ]; then
		...
		echo "    git -C smatch reset --hard 27ae97bb4cef95609a8612e5628d1419192f1754"
