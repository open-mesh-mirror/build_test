...
DEFAULT_LINUX_VERSIONS=$(echo linux-3.{2..19} linux-4.{0..8})
LINUX_VERSIONS=${LINUX_VERSIONS:=${DEFAULT_LINUX_VERSIONS}}
LINUX_DEFAULT_VERSION=${LINUX_DEFAULT_VERSION:="linux-4.8"}
...
check_external()
{
	if [ ! -x "${CGCC}" -o ! -x "${SPARSE}" ]; then
		...
		echo "    git -C sparse reset --hard 40791b94c56b1a6da2a0ddeb1f9d5c9d64de8f93"
	...
	if [ ! -x "${CPPCHECK}" ]; then
		...
		echo "    git clone -b 1.76.1 git://github.com/danmar/cppcheck.git cppcheck"
	...
	if [ ! -x "${SMATCH_CGCC}" -o ! -x "${SMATCH}" ]; then
		...
		echo "    git -C smatch reset --hard 78b2ea64f3dc9eeef3195d50c5848fc39b0d9d35"
